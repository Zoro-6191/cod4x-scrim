...
void Cbuf_AddText( const char *text )
{
	...
	if ( len + cmd_text.cursize > cmd_text.maxsize )
	{
		...
		else new_buf = realloc(cmd_text.data, len + cmd_text.cursize);
...
qboolean Cmd_AddCommandGeneric( const char *cmd_name, const char* helptext, xcommand_t function, qboolean warn, int power )
{
	...
	else cmd = S_Malloc( sizeof( cmd_function_t ) + strlen(cmd_name) + 1);
...
static void Cmd_TokenizeString2( const char *text_in, qboolean ignore_quotes ) 
{
	...
	if(tokenStrings.currentString < MAX_TOKENIZE_STRINGS)
	{
		...
		else param.cmd_tokenized = tokenStrings.cmd_tokenized;
	...
	else Com_PrintError(CON_CHANNEL_SYSTEM,"Cmd_TokenizeString(): MAX_TOKENIZE_STRINGS exceeded\n");
}
...
static void SV_Cmd_TokenizeString2( const char *text_in, qboolean ignore_quotes ) 
{
	...
	if(sv_tokenStrings.currentString < MAX_TOKENIZE_STRINGS)
	{
		...
		else param.cmd_tokenized = sv_tokenStrings.cmd_tokenized;
...
static void Cmd_List_f( void ) 
{
	...
	for ( cmd = cmd_functions ; cmd ; cmd = cmd->next ) 
	{
		if ( (match && !Com_Filter( match, (char*)cmd->name, qfalse )) || Cmd_GetInvokerPower() < cmd->minPower || ((cmd->minPower == 0) && Cmd_GetInvokerPower() != 100))
