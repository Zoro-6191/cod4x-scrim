...
const char* Cvar_VariableStringBuffer( const char *var_name, char *buffer, int bufsize ) 
{
	...
	if (!var)
		*buffer = 0;
	else Cvar_DisplayableValueMT(var, buffer, bufsize );
	Sys_LeaveCriticalSection(CRITSECT_CVAR);
	return buffer;
}
...
static cvar_t *Cvar_Register(const char* var_name, cvarType_t type, unsigned short flags, CvarValue value, CvarLimits limits, const char *description)
{
	...
	else if ( var ) 
	{
		...
		*/
		...
		else var->description = nullstring;
...
void Cvar_SetWithType( const char *var_name, CvarValue value, cvarType_t type, qboolean force )
{
	...
	else Cvar_Register( var_name, CVAR_STRING, 0, value, limits, "External Cvar");
	Sys_LeaveCriticalSection(CRITSECT_CVAR);
}
/*
============
...
void Cvar_SetAllowCheatOnly( const char *var_name, const char *value) 
{
	...
	else Cvar_Set2 (var_name, value, qfalse);
}
...
void Cvar_SetCheatState( void )
{
    ...
    for ( var = cvar_vars ; var ; var = var->next )
	{
		if ( var->flags & CVAR_CHEAT )
		{
			...
			else Cvar_SetVariant( var, cval , qtrue);
		}
    }
		Sys_LeaveCriticalSection(CRITSECT_CVAR);
	...
	switch(cvar->type)
	{
		case CVAR_BOOL:
			if(bufvalue)
			{
				...
				else Com_sprintf(bufvalue, sizevalue, "0");
			}
			if(bufreset)
			{
				if(cvar->resetBoolean)
					Com_sprintf(bufreset, sizereset, "1");
				else Com_sprintf(bufreset, sizereset, "0");
			}
			if(buflatch)
			{
				if(cvar->latchedBoolean)
					Com_sprintf(buflatch, sizelatch, "1");
				else Com_sprintf(buflatch, sizelatch, "0");
			}
			return;
		case CVAR_FLOAT:
		...
		case CVAR_ENUM:
			if(cvar->limits.enumStr)
			{
				if(bufvalue)
				{
					...
					else Com_sprintf(bufvalue, sizevalue, "Out of Range value");
				}
				if(bufreset)
				{
					...
					else Com_sprintf(bufreset, sizereset, "Out of Range value");
				}
				if(buflatch)
				{
					...
					else Com_sprintf(buflatch, sizelatch, "Out of Range value");
				}
			}
			return;
		default:
...
qboolean Cvar_Command( void )
{
	...
	if ( Cmd_Argc() == 1 ) 
	{
		...
		else Com_Printf(CON_CHANNEL_CVAR,"\"%s\" is: \"%s" S_COLOR_WHITE "\" default: \"%s" S_COLOR_WHITE "\"\n", v->name, value, reset );
...
void Cvar_List_f( void ) 
{
	...
	for (var = cvar_vars ; var ; var = var->next, i++)
	{
		...
		else Com_Printf(CON_CHANNEL_DONT_FILTER," ");
...
char	*Cvar_InfoString( int bit ) 
{
	...
	for (var = cvar_vars ; var ; var = var->next) 
	{
		if (var->flags & bit) 
		{
			if(var->type != CVAR_BOOL)
				Cvar_ValueToStr(var, value, sizeof(value), NULL, 0, NULL, 0);
			else Com_sprintf(value, sizeof(value), "%d", var->boolean);
...
char	*Cvar_DisplayableValue( cvar_t const *var) 
{
	...
	if(!var)
		value[0] = '\0';
	else Cvar_ValueToStr(var, value, sizeof(value), NULL, 0, NULL, 0);
	return value;
}
...
char *Cvar_DisplayableValueMT( cvar_t const *var, char* value, int maxlen) 
{
	if(!var)
		value[0] = '\0';
	else Cvar_ValueToStr(var, value, maxlen, NULL, 0, NULL, 0);
	return value;
}
