...
void Scr_PrintPrevCodePosSpreadSheet(conChannel_t channel, const char *codePos, bool summary, bool functionSummary)
{
    ...
    if ( codePos )
    {
        ...
        else if ( gScrVarPub.programBuffer && Scr_IsInOpcodeMemory(codePos) )
        {
            ...
            else Scr_PrintSourcePosSpreadSheet(channel, gScrParserPub.sourceBufferLookup[bufferIndex].buf, gScrParserPub.sourceBufferLookup[bufferIndex].sourceBuf, Scr_GetPrevSourcePos(codePos - 1, 0));
        }
        else Com_Printf(channel, "%s\n", codePos);
    }
    else Com_Printf(channel, "<frozen thread>\n");
}
