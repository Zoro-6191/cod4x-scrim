		...
		if(search->pack->hashTable[hash])
		{
			...
			do
			{
				...
				if(!FS_FilenameCompare(pakFile->name, filename))
				{
					...
					else fsh[*file].handleFiles.file.z = pak->handle;
...
void FS_Path_f( void ) 
{
	...
	for (s = fs_searchpaths; s; s = s->next) 
	{
		...
		else Com_Printf (CON_CHANNEL_DONT_FILTER, "%s%c%s\n", s->dir->path, PATH_SEP, s->dir->gamedir );
	}
...
int FS_Read( void *buffer, int len, fileHandle_t f ) 
{
	...
	if (fsh[f].zipFile == qfalse) 
	{
		...
		while (remaining) 
		{
			...
			if (read == 0)
			{
				...
				if (!tries)
					tries = 1;
				else return len-remaining;	//Com_Error (ERR_FATAL, "FS_Read: 0 bytes read");
			}
	...
	else return unzReadCurrentFile(fsh[f].handleFiles.file.z, buffer, len);
}
...
int FS_Read2( void *buffer, int len, fileHandle_t f ) 
{
	...
	else return FS_Read( buffer, len, f);
}
/*
=================
FS_Write
...
static pack_t *FS_LoadZipFile( char *zipfile, const char *basename ) 
{
	...
	else pack->pure_checksum = pack->checksum;
...
qboolean FS_GameDirDomainFunc(const char *cvar_name, const char *fs_gamestring)
{
	...
	if ( Q_stricmpn(fs_gamestring, "mods", 4) || strlen(fs_gamestring) < 6 || (fs_gamestring[4] != '/' && fs_gamestring[4] != '\\') || strstr(fs_gamestring, "..") ||
...
void FS_AddGameDirectory_Single(const char *path, const char *dir_nolocal, qboolean localized, int index)
{
	...
	else Q_strncpyz(dir, dir_nolocal, sizeof(dir));
	Sys_EnterCriticalSection(CRITSECT_FILESYSTEM);
    ...
    else Q_strncpyz(fs_gamedir, dir, 256);
...
fileHandle_t __cdecl FS_FOpenFileAppend( const char *filename ) 
{
	...
	else f = FS_HandleForFileForThread(3);
...
void DB_BuildQPath(const char *filename, int ffdir, int len, char *buff)
{
    ...
    switch(ffdir)
    {
        case 0:
        	...
        	else Com_sprintf(buff, len, "zone/%s.ff", filename);
            return;
...
void FS_ReferencedPaks(char *outChkSums, char *outPathNames, int maxlen)
{
	...
	if ( fs_gameDirVar->string[0] )
		for ( search = fs_searchpaths; search; search = search->next )
			if ( search->pack && !search->localized )
				if ( !(search->pack->referenced & FS_GENERAL_REF) && (!Q_stricmp(search->pack->pakGamename, fs_gameDirVar->string) || 
...
char __cdecl FS_SanitizeFilename(const char *filename, char *sanitizedName, int sanitizedNameSize)
{
	...
	while ( filename[srcIndex] )
	{
		...
		if ( filename[srcIndex] != '.' || (filename[srcIndex + 1] != 0 && filename[srcIndex + 1] != '/' && filename[srcIndex + 1] != '\\' ))
		{
			...
			else sanitizedName[dstIndex] = filename[srcIndex];
			++dstIndex;
		}
		++srcIndex;
	}
...
int __cdecl FS_GetFileList(const char *path, const char *extension, int behavior, char *listbuf, int bufsize)
{
	...
	*listbuf = 0;
	...
	else result = FS_GetModList(listbuf, bufsize);
	return result;
}
...
int FS_WriteLog( const void *buffer, int ilen, fileHandle_t h )
{
	...
	else remaining = (fhd->rbufferPos - fhd->bufferPos) -1;
...
void FS_WriteLogFlush( fileHandle_t h ) //This function gets called from the logwrite thread and fileclose function
{
	...
	else remaining = DEFAULT_LOGFILEBUFFER - (fhd->rbufferPos - fhd->bufferPos);
