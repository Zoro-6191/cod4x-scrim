...
void DB_ReadXFileStage()
{
    ...
    if ( !DB_ReadData() )
    {
        if(_GetLastError() == 38)
            g_load.ateof = true;
        else Com_Error(ERR_DROP, "Read error of file '%s'", g_load.filename);
    }
}
...
void __cdecl DB_LoadXFileData(byte *pos, int size)
{
    ...
    while( size + g_load.deflateBufferPos > DEFLATE_BUFFER_SIZE && size > 0)
    {
        ...
        if ( g_load.deflateBufferPos < DEFLATE_BUFFER_SIZE )
        {
            ...
            else memcpy(pos, &g_load.deflateBuffer[g_load.deflateBufferPos], bytesToCopy);
        ...
        else lastDeflateRemainingFileSize = DEFLATE_BUFFER_SIZE;
        ...
        if ( lastAvailOutSize == 1 )
            *pos = g_load.deflateBuffer[g_load.deflateBufferPos];
        else memcpy(pos, &g_load.deflateBuffer[g_load.deflateBufferPos], lastAvailOutSize);
        g_load.deflateBufferPos += lastAvailOutSize;
        pos += lastAvailOutSize;
    }
    if ( size == 1 )
        *pos = g_load.deflateBuffer[g_load.deflateBufferPos];
    else memcpy(pos, &g_load.deflateBuffer[g_load.deflateBufferPos], size);
...
void __cdecl DB_LoadXFileInternal()
{
    ...
    if ( version != FASTFILE_VERSION )
    {
        ...
        else Com_Error(ERR_DROP, "Fastfile for zone '%s' is out of date (version %d, expecting %d)", g_load.filename, version, FASTFILE_VERSION);
    }
...
void __cdecl Load_XString(bool atStreamStart)
{
    ...
    if ( *varXString )
    {
        ...
        else DB_ConvertOffsetToPointer(varXString);
    }
}
...
void __cdecl Load_TempString(bool atStreamStart)
{
    ...
    if ( *varTempString )
    {
      ...
      else DB_ConvertOffsetToPointer(varTempString);
    }
}
...
void Load_XModelBoneNames()
{
    if ( varXModel->boneNames )
    {
        ...
        else DB_ConvertOffsetToPointer(&varXModel->boneNames);
    }
}
...
bool DB_GetQPathForZone(const char* zonePath, int maxlen, char* opath)
{
    ...
    else Q_strncpyz(zoneName, zonePath, sizeof(zoneName));
...
void __cdecl Load_WeaponDef(bool atStreamStart)
{
    ...
    if ( v12 )
    {
        ...
        else DB_ConvertOffsetToPointer(&varWeaponDef->bounceSound);
        v11 = varWeaponDef;
    }
...
void __cdecl DB_UnloadXZoneInternal(unsigned int zoneIndex, bool createDefault)
{
    ...
    else Com_Printf(CON_CHANNEL_SYSTEM, "and deleting all assets\n");
...
LABEL_48:
            ...
            while ( *pOverrideAssetEntryIndex )
            {
                ...
                else pOverrideAssetEntryIndex = &overrideAssetEntry->entry.nextOverride;
            }
            pAssetEntryIndex = &assetEntry->nextHash;
        }
    }
...
void __cdecl DB_PrintXAssetsForType(XAssetType type, void (__cdecl *func)(XAssetHeader, void *), void *inData, bool includeOverride)
{
    ...
    else DB_EnumXAssets(type, func, &inData, includeOverride);
}
...
void __cdecl DB_LogMissingAsset(XAssetType type, const char *name)
{
    ...
    else g_missingAssetFile = FS_FOpenFileWrite("missingasset.csv");
    if ( g_missingAssetFile )
    {
...
XAssetEntryPoolEntry *__cdecl DB_CreateDefaultEntry(XAssetType type, const char *name)
{
    ...
    if ( !asset.header.xmodelPieces )
    {
        ...
        else Com_Error(ERR_DROP, "Couldn't find the bsp for this map.  Please build the fast file associated with %s and try again.", name);
    }
    asset.type = type;
    ++g_defaultAssetCount;
    ...
    if ( db_xassetdebug->boolean )
    {
        ...
        if ( db_xassetdebugtype->integer == -1 || db_xassetdebugtype->integer == type || (db_xassetdebugname[0] && !Q_stricmp(name, db_xassetdebugname)) )
...
void __cdecl PrintWaitedError(XAssetType type, const char *name, int waitedMsec)
{
  if ( waitedMsec > 100 )
  {
      ...
      else Com_PrintError(CON_CHANNEL_ERROR, "Waited %i msec for missing asset \"%s\".\n", waitedMsec, name);
  }
  switch ( type )
  {
      case ASSET_TYPE_XANIMPARTS:
      ...
      case ASSET_TYPE_LOCALIZE_ENTRY:
        if ( loc_warnings->boolean )
        {
            ...
            else Com_PrintWarning(CON_CHANNEL_FILES, "Could not load %s \"%s\".\n", g_assetNames[type], name);
        }
        break;
...
XAssetHeader __cdecl DB_FindXAssetHeader(XAssetType type, const char *name, bool errorIfMissing, int waitTime)
{
    ...
    if ( !Q_strncmp(so_prefix, name, strlen(so_prefix)) )
    {
        ...
        if ( namelen > bspextlen && !Q_strncmp(bspext, &name[namelen - bspextlen], bspextlen) )
        {
            ...
            else Com_sprintf(so_name, sizeof(so_name), "maps/mp/%s", basename + 1);
            use_so_name = 1;
        }
    }
    ...
    do
    {
        while ( 1 )
        {
            ...
            else ent = DB_FindXAssetEntry(type, name);
        ...
        if ( DB_IsMinimumFastFileLoaded() && DB_GetInitializing() )
            break;
        if ( Sys_IsDatabaseReady() && (Sys_IsMainThread() /* || Sys_IsRenderThread() && R_IsInRemoteScreenUpdate() && Sys_QueryRGRegisteredEvent()*/))
            DB_PostLoadXZone();
        else
        {
    ...
    if ( errorIfMissing && type != ASSET_TYPE_XANIMPARTS )
    {
        if ( com_developer->boolean )
        {
            ...
            else DB_LogMissingAsset(type, name);
        }
    }
    if ( start )
    {
    ...
    switch ( type )
    {
        ...
        default:
            ...
            else newEntryPoolEntry = DB_CreateDefaultEntry(type, name);
...
void __cdecl DB_DelayedCloneXAsset(XAssetEntry *newEntry)
{
    ...
    if ( g_sync )
    {
        if ( db_xassetdebug->boolean )
        {
            ...
            if ( db_xassetdebugtype->integer == -1 || db_xassetdebugtype->integer == newEntry->asset.type || (debugname[0] && !Q_stricmp(debugname, assetname)) )
            {
                assetname = DB_GetXAssetName(&newEntry->asset);
...
XAssetEntryPoolEntry *__cdecl DB_LinkXAssetEntry(XAssetEntry *argNewEntry, int allowOverride)
{
    ...
    {
        debugname = Cvar_GetVariantString("db_xassetdebugname");
        if ( db_xassetdebugtype->integer == -1 || db_xassetdebugtype->integer == newEntry->entry.asset.type || (debugname[0] && !Q_stricmp(name, debugname)) )
            ...
            if ( !Q_stricmp(locname, name) )
            {
                if ( db_xassetdebug->boolean )
                {
                    ...
                    if ( db_xassetdebugtype->integer == -1 || db_xassetdebugtype->integer == newEntry->entry.asset.type || (debugname[0] && !Q_stricmp(name, debugname)) )
                    {
                        ...
                        else poolname = "default asset pool";
    ...
    if ( !allowOverride || q)
    {
        ...
        else DB_DelayedCloneXAsset(&newEntry->entry);
        if ( db_xassetdebug->boolean )
        {
            debugname = Cvar_GetVariantString("db_xassetdebugname");
            if ( db_xassetdebugtype->integer == -1 || db_xassetdebugtype->integer == existingEntry->entry.asset.type || (debugname[0] && !Q_stricmp(name, debugname)) )
            {
                if ( g_zones[existingEntry->entry.zoneIndex].zoneinfo.name[0] )
                    poolname = g_zones[existingEntry->entry.zoneIndex].zoneinfo.name;
                else poolname = "default asset pool";
